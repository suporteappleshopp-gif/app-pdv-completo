---
name: lasy-cloud
description: Lasy Cloud - Full Supabase project management via CLI. Use for creating tables, RLS policies, auth, migrations, and database operations.
---
...
- **DATABASE_URL**: PostgreSQL connection string (migrations, Prisma)
- **SUPABASE_PROJECT_REF**: Project reference for `npx supabase link`
- **SUPABASE_DB_PASSWORD**: Database password for `npx supabase link`
...
**Regra para variáveis no código do usuário:** Sempre defina no código do usuário (e no .env quando documentar) variáveis de ambiente com o prefixo **VITE_**. O Vite só expõe ao client variáveis que começam com `VITE_`; outras não estarão disponíveis em `import.meta.env`. Exemplos: `VITE_API_URL`, `VITE_SUPABASE_URL`, `VITE_LASY_PROJECT_ID`.
...
## Migrations with Prisma (DATABASE_URL)

`DATABASE_URL` is already set in this environment. You can use Prisma for migrations:
...
### 2. Define schema in `prisma/schema.prisma`, then run migrations
```bash
# Create migration from schema changes
npx prisma migrate dev --name create_products

# Or push schema without migration files (no prompt)
npx prisma db push
```

Use `npx prisma db push` in this sandbox to avoid interactive prompts.
