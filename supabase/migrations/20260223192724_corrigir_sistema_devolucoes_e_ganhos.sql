-- ===============================================
-- CORREÇÃO COMPLETA DO SISTEMA DE DEVOLUÇÕES E GANHOS
-- ===============================================
...
BEGIN
  ...
  IF NEW.tipo_destino = 'estoque' THEN
    ...
    WHERE nome = NEW.produto_nome AND operador_id = NEW.operador_id;
...
BEGIN
  ...
  RETURN QUERY
  SELECT COALESCE(SUM(v.valor_total), 0)::numeric as total_ganhos
  FROM vendas v
...
SELECT
  ...
  SUM(v.valor_total) as ganhos_do_dia,
...
ON vendas(operador_id, created_at DESC, valor_total);

CREATE INDEX IF NOT EXISTS idx_avarias_operador_tipo
ON avarias(operador_id, tipo_destino, created_at DESC);
...
BEGIN
  ...
  SELECT
    ...
    COUNT(*) FILTER (WHERE valor_total IS NULL OR valor_total = 0)::bigint as vendas_sem_valor,
...
-- 10. Atualizar RLS para garantir que usuários só vejam seus próprios dados
-- Política para view_ganhos_usuario
DROP POLICY IF EXISTS "Usuários podem ver apenas seus próprios ganhos" ON vendas;
CREATE POLICY "Usuários podem ver apenas seus próprios ganhos"
  ON vendas FOR SELECT
  USING (operador_id = auth.uid()::text);
...
BEGIN
  ...
  RAISE NOTICE '   - O painel de avarias será removido do frontend';
  RAISE NOTICE '   - Análise de ganhos mostrará apenas vendas';
  RAISE NOTICE '   - Todas as vendas serão salvas automaticamente no Supabase';
END $$;
